...
import {CommonApiResponse, Iuser, Ialert} from "@/commonType";

const Login = () => {
  const { isAuth, setIsLogin, loginAuth } = useContext(LoginAuthContext)
  ...
  const onSubmit = async (e:MouseEvent<HTMLButtonElement>) => {
    ...
    if (accountIsValid && passwordIsValid) {
      ...
      try {
        const res = await axios.post<CommonApiResponse<Iuser>&{token: string}>('https://l8-upgrade-apis.herokuapp.com/api/login', params)
        if (res.data.success) {
          setIsLogin(true)
